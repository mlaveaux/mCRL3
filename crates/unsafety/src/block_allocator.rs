use std::{alloc::Layout, array, fmt, mem::ManuallyDrop, ptr::{self, NonNull}, sync::Mutex};
...
impl<T, const N: usize> BlockAllocator<T, N> {
    ...
    pub fn allocate_object(&mut self) -> Result<NonNull<T>, AllocError> {
        ...
        unsafe {
            ...
            Ok(NonNull::new_unchecked(&mut block.data[length].data as *mut ManuallyDrop<T> as *mut T))
        }
    }
...
impl<T, const N: usize> Block<T, N> {
    fn new() -> Self {
        Self {
            data: array::from_fn(|_i| Entry { next: NonNull::dangling() }),
            length: 0,
            next: None,
        }
    }
