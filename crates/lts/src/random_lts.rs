...
mod tests {
    ...
    /// A simple wrapper around `kani::any` to implement the `TryRng` trait, which is required by the random LTS generation functions.
    struct ArbRng {}
    ...
    fn random_lts_test() {
        let mut rng = ArbRng {};
        let lts = random_lts(&mut rng, 0, 0, 0);
    }
}
