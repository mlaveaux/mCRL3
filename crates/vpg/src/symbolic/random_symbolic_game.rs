...
/// at random into `num_groups` transition relations, each reading and writing
/// *every* position (`0..k`): a short relation that only reads a subset of
/// positions is a projection, which adds spurious edges for any two states that
/// agree on the read positions — correct when encoding a real LPS, wrong here,
/// where the input is an arbitrary graph with no such structure to exploit.
///
...
pub fn encode_parity_game<G: PG, R: Rng>(
    ...
    let mut edge_buckets: Vec<Vec<Vec<Value>>> = vec![Vec::new(); num_groups];
    for v in 0..num_of_vertices {
        for edge in game.outgoing_edges(VertexIndex::new(v)) {
            ...
            let bucket = rng.random_range(0..num_groups);
    ...
    let symbolic_game = SymbolicParityGame::new(manager, &groups, [even_vertices, odd_vertices], priorities)?;
