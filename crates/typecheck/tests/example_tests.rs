...
//! the data specification. The `#[test_case]`s commented out below with no other comment are all
//! rejected for the same reason: mCRL2's concrete syntax overloads several tokens between the
//! process algebra and the data language — most commonly `.` (process sequential composition vs.
//! the data "at"/indexing operator) and `+` (process choice vs. data addition) — and
//! disambiguating between the two readings needs semantic information a context-free grammar
//! doesn't have. `crate::process::check`'s `check_condition` recovers the common case (a single
//! action or process-instantiation step swallowed into an `if`'s condition via `.`), but not
//! every shape this ambiguity produces — most commonly, a `+`-separated chain of guarded
//! alternatives (`cond1 -> P1 + cond2 -> P2 + ...`) where the guard bodies contain actions. Fully
//! resolving this would need either a semantics-directed reparse or a grammar change in
//! `merc_syntax`, both out of scope here; see the crate README.
