...
mod tests {
    ...
    ///
    /// GAP enumerates the whole group and minimises directly, so it shares no
    /// idea with the stabilizer chain it is used to check — that is the point.
    /// It is also why callers must keep the group small enough to enumerate.
    fn gap_lex_min(gens: &[Permutation], n: usize, states: &[Vec<usize>]) -> Vec<Vec<usize>> {
        ...
        let states_joined = states.iter().map(|s| format!("[{}]", s.iter().format(","))).format(",\n  ");
...
"#
    ...
    /// Random groups checked against GAP.
    ///
    /// The handwritten groups above were all taken from PBESs that had already
    /// been looked at, so they share whatever blind spot led to picking them;
    /// here the group and the values are drawn independently of the code.
    ///
    /// Both chains are checked, because the two disagree on the base — GAP is
    /// asked for `[1..n]` while Schreier–Sims picks the first moved point — and
    /// a random group is what makes the bases actually diverge.
    #[test]
    ...
    /// Canonicalization has to be constant on orbits — that is what makes the
    /// quotient sound — so every image of a random state must reach the same
    /// representative as the state itself.
    #[test]
