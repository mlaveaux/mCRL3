...
impl DensePerm {
    ...
    pub(crate) fn compose(alpha: &DensePerm, beta: &DensePerm) -> Self {
...
///
/// `base_point` is the point β fixed by the stabilizer at this level.
/// `transversal[x]` is the unique coset representative `u` such that `u(base_point) == x`.
pub(crate) struct SchreierLevel {
    pub(crate) base_point: usize,
    /// Maps orbit point → coset representative (a `DensePerm` mapping `base_point → orbit_point`).
    pub(crate) transversal: HashMap<usize, DensePerm>,
}
...
impl Bsgs {
    ...
    /// `0..param_offset` (the equation index) are never permuted.
    /// 
    /// # Details
    /// 
    /// Pruned transversal walk from the paper.
    ///
    /// Cost: Σ |U_i| permutation compositions, versus |orbit|·n for Stage A.
    pub(crate) fn canonicalize(&self, state: &[usize], param_offset: usize) -> Vec<usize> {
        ...
        // `current` = set of candidate permutations achieving the lex-min prefix so far.
        let mut current: Vec<DensePerm> = vec![DensePerm::identity(self.n)];

        for level in &self.chain {
            let bp = level.base_point;
            let mut next_current: Vec<DensePerm> = Vec::new();
            let mut best_value = usize::MAX;

            for g in &current {
                for u in level.transversal.values() {
                    // h = u ∘ g  (g applied first, then u)
                    let h = DensePerm::compose(u, g);

                    // The value at position `bp` under h's positional reordering.
                    // h.apply_to_vec(params)[bp] == params[h_inv(bp)], but we can
                    // compute it as: which i maps to bp under h?  i.e. h_inv(bp).
                    // Equivalently: among all i, h.apply(i)==bp → value = params[i].
                    // Fastest: invert on the fly just for one point.
                    let source = h.inverse().apply(bp);
                    let new_value = if source < params.len() { params[source] } else { usize::MAX };

                    if new_value < best_value {
                        best_value = new_value;
                        next_current = vec![h];
                    } else if new_value == best_value {
                        next_current.push(h);
                    }
                }
            }
        ...
        // Among remaining candidates, find the lex-min image.
        let mut min_params: Option<Vec<usize>> = None;
        for g in &current {
            let candidate = g.apply_to_vec(params);
            if min_params.as_ref().map_or(true, |m| candidate < *m) {
    ...
    fn all_generators(&self) -> Vec<DensePerm> {
        let mut gens: HashSet<DensePerm> = HashSet::new();
        for level in &self.chain {
            for u in level.transversal.values() {
                gens.insert(u.clone());
...
fn permutation_to_gap_cycles(perm: &Permutation, n: usize) -> String {
    let dense = DensePerm::from_permutation(perm, n);
...
fn parse_schreier_level(s: &str, n: usize) -> Result<SchreierLevel, MercError> {
    ...
    Ok(SchreierLevel { base_point, transversal })
}
...
fn parse_transversal_list(s: &str, base_point: usize, n: usize) -> Result<HashMap<usize, DensePerm>, MercError> {
    let s = s
        .trim()
        .strip_prefix('[')
        .and_then(|s| s.strip_suffix(']'))
...
fn parse_gap_perm(s: &str, n: usize) -> Result<DensePerm, MercError> {
    let s = s.trim();
    if s == "()" {
        return Ok(DensePerm::identity(n));
    }
...
fn bsgs_schreier_sims(gens: &[Permutation], n: usize) -> Result<Bsgs, MercError> {
    let dense_gens: Vec<DensePerm> = gens.iter().map(|p| DensePerm::from_permutation(p, n)).collect();
    let chain = schreier_sims_chain(&dense_gens, n);
    Ok(Bsgs { n, chain })
}

fn schreier_sims_chain(gens: &[DensePerm], n: usize) -> Vec<SchreierLevel> {
    if gens.is_empty() {
        return vec![];
    }
    ...
    let base_point = (0..n)
        .find(|&x| gens.iter().any(|g| g.apply(x) != x))
        .unwrap_or(0);
    ...
    let mut chain = vec![SchreierLevel { base_point, transversal }];
...
fn compute_orbit_transversal(gens: &[DensePerm], base_point: usize, n: usize) -> HashMap<usize, DensePerm> {
    let _ = n;
    let mut transversal: HashMap<usize, DensePerm> = HashMap::new();
    let mut queue: VecDeque<usize> = VecDeque::new();

    transversal.insert(base_point, DensePerm::identity(gens[0].n()));
    ...
    while let Some(x) = queue.pop_front() {
        ...
        for g in gens {
            ...
            if !transversal.contains_key(&y) {
                let rep_y = DensePerm::compose(g, &rep_x);
                transversal.insert(y, rep_y);
                queue.push_back(y);
            }
        }
    }
...
fn schreier_generators(
    gens: &[DensePerm],
    transversal: &HashMap<usize, DensePerm>,
    _base_point: usize,
) -> Vec<DensePerm> {
    let mut result: Vec<DensePerm> = Vec::new();
    ...
    for (&x, u_x) in transversal {
        for s in gens {
            ...
            // schr = u_x · s · u_sx^{-1}
            let schr = DensePerm::compose(&u_sx_inv, &DensePerm::compose(s, u_x));
...
mod tests {
    ...
    fn dense_perm_compose_and_inverse() {
        ...
        let swap = DensePerm::from_permutation(&gens[0], n);
        let rot = DensePerm::from_permutation(&gens[1], n);

        // swap ∘ rot: first rot then swap
        let composed = DensePerm::compose(&swap, &rot);
        ...
        let id = DensePerm::compose(&swap, &inv);
    ...
    fn permutation_to_gap_cycles_roundtrip() {
        ...
        let dense = DensePerm::from_permutation(&p, 4);
